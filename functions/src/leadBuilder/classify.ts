...
const EXEMPT =
  /\b(CITY OF|COUNTY OF|TOWN OF|VILLAGE OF|STATE OF|SCHOOL|COLLEGE|UNIVERSIT|CHURCH|PARISH|DIOCESE|TEMPLE|LODGE|CLUB|AUTHORITY|HOUSING|HDFC|URBAN RENEWAL|FIRE (CO|DEPT|DIST)|CREDIT UNION|\bCU\b|FOUNDATION|MINISTR|NATION OF INDIANS|PASNY|NYPA|POWER AUTH)\b/;
...
const WORD =
  /\b(CO|COMPANY|LTD|LIMITED|LP|PC|PA|ASSOC|ASSOCIATES|ENTERPRISES|HOLDINGS|PROPERTIES|REALTY|GROUP|PARTNERS|PARTNERSHIP|VENTURES|INDUSTRIES|MANUFACTURING|SERVICES|SYSTEMS|SUPPLY|EQUIPMENT|CONSTRUCTION|CONTRACTING|MOTORS|FOODS|MARKET|STORES|PLAZA|BANK|FUND|TRUST)\b/;
...
export function normalizeName(last: string, first: string): string {
...
export type EntityClass = 'COMPANY' | 'PERSON' | 'EXEMPT' | 'REVIEW';
 ...
 */
export function classifyEntity(last: string, first: string): { cls: EntityClass; name: string } {
  const name = normalizeName(last, first);
  if (EXEMPT.test(name)) return { cls: 'EXEMPT', name };
  if (SUBSTR.test(name) || WORD.test(name)) return { cls: 'COMPANY', name };
  const toks = name.split(' ').filter(Boolean);
  const hasDigit = /\d/.test(name);
  if (first.trim() && !hasDigit && toks.length <= 3) return { cls: 'PERSON', name };
  if (!first.trim() && PERSON_PAT.test(name) && toks.length <= 3 && !hasDigit) return { cls: 'PERSON', name };
  return { cls: 'REVIEW', name };
}

export function tierFor(marketValue: number): 'GIANT' | 'BIG' | 'MID' | 'SMALL' {
  if (marketValue >= 5_000_000) return 'GIANT';
  if (marketValue >= 1_500_000) return 'BIG';
  if (marketValue >= 500_000) return 'MID';
  return 'SMALL';
}

/** Landlord-ish name (incl. address-named SPE LLCs like "123 Main St LLC") — routing hint only. */
export function isLandlordName(normalizedName: string): boolean {
