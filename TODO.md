...
**Leads / Sales CRM overhaul (feat/leads-sales-ux, v1.76.0 — 2026-06-19)**

- [x] **Sales-rep UX overhaul SHIPPED v1.76.0.** Single filterable pipeline (status-filter chips, Archive tab folded in), Location column + carried `parcelAddress`/`mailingAddress`/`city`/`state` from Lead Builder promotion, profile with Open-in-Maps link, additive `additionalContacts[]`/`altPhones[]` (enriched fields stay read-only), named-slot documents (Bill/Contract/Other → Storage `leads/{leadId}/…`), per-rep Stats, Reopen-closed-lead action, pure-rep landing on `/sales-crm`. Bulk CSV upload removed. Inline-array writes use atomic `arrayUnion`/`arrayRemove`. (source: conversation 2026-06-19 — JB brain dump)
- [ ] **Deploy `storage.rules`** — the new `leads/{path=**}` Storage rule (authenticated employee, <50MB) must be deployed for document upload to work in prod: `firebase deploy --only storage`. (source: conversation 2026-06-19)
- [ ] **Mobile-reveal stuck-pending fallback (review finding #4, LOW)** — if the Apollo webhook never fires, a lead sits on "Revealing mobile…" forever with no retry. Consider a staleness fallback (pending for >N min → re-show "Grab number"); guard against double-charging an Apollo credit (the callable returns `alreadyRevealed`, so a retry should be safe). (source: review 2026-06-19)
- [ ] **Per-rep document isolation (review finding #3, MEDIUM, accepted-for-now)** — Storage `leads/` is the platform's standard authenticated-employee floor, so a rep can technically read/write another rep's lead blobs in Storage even though Firestore blocks the lead doc. Matches the documented posture for `documents/`; revisit with signed-URL serving if per-rep isolation becomes a requirement. (source: review 2026-06-19)

**Investor one-pager / Site Briefing (feat/investor-onepager, v1.74.0 — 2026-06-19)**

- [x] **Manual `zoning` field (option a) — SHIPPED v1.74.0.** Added `zoning?: string` to `SiteRegistryEntry`, a "Zoning (from LandID)" input in `DetailEditForm.tsx` (saved via `SiteAnalyzerDetail.handleSave`), a read-view row in `DetailSummary.tsx`, and a **Zoning** row in `landUseSection` (`executiveSummary.ts`) that shows "—" until set (no fallback to `priorUsage` — a land use isn't a zoning class). **Action for JB: populate Zoning per site from LandID** (e.g. "Unzoned", "I-2 Heavy Industrial", "AG"). (source: conversation 2026-06-19)
- [ ] **Zoning auto-pull (option b, optional)** — integrate a Regrid/LandID parcel API keyed by `parcelId`/coords to auto-fill `zoning` instead of manual entry. Only worth it if manual entry becomes a bottleneck. (source: conversation 2026-06-19)
- [x] **Briefing redesigned to "The Verdict" (v1.75.0)** — after a deep-research pass. Leads with deliverable MW + a GO/CONDITIONAL/NO-GO verdict badge (engineer-reviewed from the linked LLR via `usePreConSiteByRegistryId`, else appraisal-suggested), power-context map band, and FAB "Why this site wins" benefit tiles. **Resolved the 6,500-vs-1,500 MW credibility risk**: the hero now prefers `engineerVerifiedMW` from the LLR over the Site-Analyzer target. (source: conversation 2026-06-19)
- [x] **Transmission lines added to the briefing map (v1.77.0)** — both `buildGridStaticMap` (PDF) and `GridContextMap` (screen) now draw voltage-colored transmission lines (white casing + purple/blue/teal) under the substation markers. Geometry is fetched live via `fetchTransmissionLines` (HIFLD; not stored on the site). Shows the grid backbone reaching the site, not just dots. (source: conversation 2026-06-19)
- [ ] **Verify the map in-browser** (lines render browser-only; headless shows a placeholder). If still not convincing, remaining proximity-context upgrades: a site→nearest-substation connector + distance label, and a metro/load-center pin. (source: conversation 2026-06-19)
- [ ] **Optional: add the "time-to-power" contrast strip** to fill the lower third — the single most persuasive non-table visual per research: a two-bar compare of the industry interconnection queue (~5.75 yrs / ~2,100 days, LBNL "Queued Up 2025" — date it on the sheet) vs. THIS site's energization year. Blends "The Verdict" with "On the Clock"; decide if wanted. (source: conversation 2026-06-19)
- [ ] **Decide the off-sheet pricing play.** The briefing intentionally carries no price (best practice — avoid anchoring/capping upside on a scarce asset). Decide how price is presented in the deal flow: "offers invited / price on request", a competitive call-for-offers, or an internal-only value model (the removed $75K–$150K/MW math can live in an internal sheet, never the buyer PDF). (source: conversation 2026-06-19)

**Lead Builder (feat/lead-builder-pipeline — 2026-06-17)**

- [x] **Build form: county/state are now dropdowns, not free-text** — backed by `TARGETABLE_REGIONS` in `src/lib/leadPipeline.ts` (NY + its 57 assessment-roll counties, exact `county_name` spellings). Scope picker removed (single option) — hardcoded to `industrial`; commercial dropped from the UI until the commercial classifier is ready (backend still understands `commercial-industrial`). (source: conversation 2026-06-17 — JB UX dump)
- [ ] **Public-power suppression v2 — per-address territory resolver** — v1 (shipped 2026-06-17) drops parcels in NY's 48 municipal-electric systems at ingest by matching `municipality_name` against `MUNI_ELECTRIC` in `ingest.ts`. Two gaps remain: (1) the 4 rural electric **co-ops** (Delaware County, Oneida-Madison, Otsego, Steuben) serve scattered rural territory with no city name to match on; (2) village-in-town precision — a muni serves a village footprint but the roll's assessing unit is often the parent town, so the city match can over/under-reach. Both are solved by a per-address serving-utility resolver — and one now EXISTS: the `feat/retail-utility-territory` resolver (PRs #158/#159, also merged 2026-06-17) resolves IOU/muni/coop at a coordinate for Site Analyzer. v2 = wire that resolver into `ingestCountyTaxRoll` (resolve each parcel's serving utility, suppress muni+coop precisely) instead of the city-name heuristic. Until then, muni drops are visible + recoverable in the Dropped tab via `ineligibleReason`. (source: conversation 2026-06-17)
...
- [x] **Backfill ran 2026-06-12**: all 49 `user-tasks` docs stamped with `archived:false` + `visibility`. Org policy forbids service-account key creation on this project, so it ran via Firestore REST with JB's gcloud user token instead of `scripts/migrate-user-tasks.mjs` (same logic — script kept in repo for future projects/restores). **JB then decided pre-existing tasks should stay private** (they were created under the private model; owners opt in by flipping individual tasks to Company): the 44 company-stamps were reverted to 'private' the same day. Net effect: legacy tasks visible only to their creators; Team/Week boards fill up as new (company-default) tasks are created. (done 2026-06-12)
