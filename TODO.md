...
- [ ] **RUN THE BACKFILL — blocking**: `GOOGLE_APPLICATION_CREDENTIALS=./service-account.json node scripts/migrate-user-tasks.mjs --confirm` (dry-run first without --confirm). Until it runs, ALL pre-existing user-tasks docs (including today's test tasks) are invisible in the tool — the main listener filters `archived == false` and legacy docs lack the field. Also stamps `visibility` so legacy tasks appear on the Team/Week boards. Needs the Firebase Admin service-account JSON (same one used for past migrations); run with `NODE_PATH=functions/node_modules` if firebase-admin isn't installed at root. (source: code review 2026-06-12)
