...
- [ ] **Publish the widened `user-tasks` Firestore rule in the Console** — replaces the v1.48 owner-only block; until then Team view / assigned tasks throw `permission denied`. Exact block in `docs/firestore-rules.md` (read+update: company-visible OR owner OR assignee; create: owner==self; delete: false). (source: conversation 2026-06-12)
- [ ] **Deploy the activity trigger**: `firebase deploy --only functions:onUserTaskWrite`. (source: conversation 2026-06-12)
