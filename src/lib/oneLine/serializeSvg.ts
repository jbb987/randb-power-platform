...
function el(p: Primitive): string {
  switch (p.kind) {
    ...
    case 'circle':
      return `<circle cx="${p.cx}" cy="${p.cy}" r="${p.r}" stroke="#000" stroke-width="${p.width ?? 1}" fill="${p.fill ?? 'none'}"${p.dash ? ` stroke-dasharray="${p.dash}"` : ''}/>`;
    case 'path':
      return `<path d="${p.d}" stroke="#000" stroke-width="${p.width ?? 1}" fill="${p.fill ?? 'none'}"${p.dash ? ` stroke-dasharray="${p.dash}"` : ''}/>`;
    case 'text':
