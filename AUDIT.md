...
- **L-2 — Security review lows (2026-06-18, branch `security/platform-hardening`):**
  ...
  - **NREL + EIA + BLS API keys were inlined into the client bundle** (free, rate-limited = low) — `fixed` (2026-06-18): all three now proxied server-side via the Cloudflare Worker (`/api/nrel` query, `/api/eia` query, `/api/bls` POST-body), mirroring the Census proxy; the Vite dev proxy injects in dev. Verified — the built bundle contains only the `/api/*` proxy paths, no API hosts and no key values. **Actions:** NREL key rotated (it was in git history); EIA/BLS were also bundle-exposed (free keys) so optionally rotate; set `VITE_NREL_API_KEY` / `VITE_EIA_API_KEY` / `VITE_BLS_API_KEY` as Worker runtime **Secrets** so they bind to the Worker.
  - **MCP bearer compare early-returns on length mismatch** → `fixed` (2026-06-18): folds the length difference into the accumulator and scans the expected token's full length — no early return.
  - **`apolloPhoneWebhook` request_id binding skippable** → `fixed` (2026-06-18): the callback must now supply a request_id that matches the lead's stored `phoneRequestId` (omitting it no longer bypasses the binding via a URL `leadId`).
  - **SoQL injection via unvalidated `rollYear`** (nySocrata.ts) → `fixed` (2026-06-18): `rollYear` is validated as a bare 4-digit year before interpolation into the `$where` clause.
