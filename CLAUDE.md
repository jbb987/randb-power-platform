...
- **Site Leads** (toolId `site-leads`, route `/site-leads`, lives in the **Pre-Construction** dashboard section) — Review queue for inbound landowner submissions from the public "Is my land powerable?" funnel (marketing site → `POST /api/public/site-score` → `site-leads` Firestore collection, server-created; clients can't create). The tool lists each submission with its coarse verdict (GO/CONDITIONAL/NO_GO) + MW range, owner contact, and a map link; staff mark under review / reject, or **Promote to Lead** (one click → writes a `leads` doc assigned to the promoting user with `source: 'site-lead'`, stamps the site-lead `qualified` + `promotedToLeadId`). Verdict thresholds (`src/lib/quickScoreVerdict.ts`) are screening heuristics pending calibration with Bailey. Lib `src/lib/siteLeads.ts`, hook `src/hooks/useSiteLeads.ts`. Phase 1 (the public endpoint + storage) shipped in v1.76.0; this is the Phase 2 review tool.
...
- **Leads (Sales CRM)** — Lead pipeline for the sales team. **Two nav views (v1.82.0): My Pipeline + Prospects.** Sales reps are independent: each gets an assigned book *and* a shared **Prospects** pool (a lead with `assignedTo === ''`; the internal concept/keys stay `pool`, the user-facing label is **Prospects** — v1.82.0). A rep sees their own leads + every prospects lead (`useLeads` visibility = `assignedTo === uid || !assignedTo`); admins see all. **Prospects view** lists unclaimed leads (tier/location shown) with a per-row **Grab** button (`assignedTo '' → me`); a rep can also **Return to prospects** one of their own leads (`me → ''`) from the LeadDetail modal. Tab icons: My Pipeline = funnel, Prospects = people/crowd. **Checkbox multi-select** (header select-all selects the *filtered* set) drives a bulk action bar: **Grab N** (Prospects), **Return N to prospects** (My Pipeline), and **Reassign N → rep** (admin only). The table also has **State + County territory filters** plus a **Tier filter** (GIANT/BIG/MID/SMALL — v1.82.3); counts on the status chips follow the chosen territory/tier, and all clear on tab switch. **Single filterable list** with status chips in a fixed order — **All · New · Call 1 · Call 2 · Call 3 · Won · Lost** (default All; the old Active chip + the per-rep **Stats** tab were removed in v1.82.0). The **Email Sent** stage was removed v1.82.0 — the sequence is now **New → Call 1 → Call 2 → Call 3 → Won/Lost** with a **Reopen → New** action on closed leads; any legacy `email_sent` lead is normalized to **Call 1** on read (`normalizeLeadStatus` in `types/index.ts`, applied in `subscribeLeads`). **Firestore rules (v1.82.0):** delete is owner-or-admin only (was: any sales-crm holder could delete any rep's lead); a rep update is limited to editing a lead they own, grabbing a pool lead, or dropping their own to the pool — they can't touch another rep's owned lead. Admins reassign freely. Leads carry an optional `mobilePhone` revealed on-demand via the **"Grab number"** button (`revealLeadPhone` callable → Apollo async webhook `apolloPhoneWebhook` → writes the decision-maker's direct mobile back). The lead **profile** (LeadDetail modal, redesigned v1.82.0) is organized into titled sections — **Company info · People info · Status · Documents · Notes** — and opens read-only with an explicit header **Edit** button (owner or admin only — a prospects lead isn't editable until grabbed) that flips the Company + People fields into a form: **company name, decision maker, role, email, phone, website, description**, **state + county as cascading dropdowns** (driven by `TARGETABLE_REGIONS` — NY today, auto-grows with new tax-roll adapters; an off-list existing value is preserved as an option), plus (admin) an **Assigned to** dropdown — all formerly-canonical Apollo/Perplexity fields are now **fully editable** (Save writes only changed fields; Cancel discards). The **Phone** area (v1.82.0) is two filled fields side by side: **Main line** (`lead.phone`, tap-to-call) and **Direct mobile** — a **Grab number** button (`revealLeadPhone` callable → Apollo webhook) that becomes the number with a **"Direct line"** tag once revealed. The main line stays editable in the Edit form. The **People info** section presents people as uniform numbered cards — **Decision maker 1** (the primary, from `lead.decisionMakerName`/role/email/phone/mobile), then **Decision maker 2, 3…** from `additionalContacts[]` — with an **Add decision maker** button below that opens a new card to fill (no separate "More contacts" heading). Company info shows the **Website** (clickable, editable — v1.82.2) and Decision maker 1 shows the **LinkedIn** link when present. Reps also attach named-slot **documents** as cards (Utility Bill / Signed Contract / Other, uploaded to Storage under `leads/{leadId}/…`). (The legacy `altPhones[]` "Other numbers" UI was removed v1.82.0; the field stays in the schema.) Inline-array writes use Firestore `arrayUnion`/`arrayRemove` (atomic, race-safe). The header keeps the size **`tier`** badge (the `energyIntensity` and `source` badges were dropped v1.82.0 as low-signal). Assignment is shown once (header owner line; reassign via the Edit form), not duplicated. Secondary/destructive actions — **Reopen** (closed leads), **Return to prospects** (own leads), **Delete** (owner/admin, with a confirm bar) — live in a header **⋮ overflow menu** (vertical dots), not the body; the Status section shows the clickable stage chips + Mark Won/Lost (or a "reopen from the ⋮ menu" note when closed). Created/Updated dates show in the header, not a footer. **On an unclaimed Prospects lead a non-owner sees a read-only profile** — the Status chips, Notes box, Document upload/remove, Add-decision-maker, and Grab-number controls are all gated behind ownership (`canEdit = admin || own`), so a rep must **Grab** first (mirrors the Firestore rule; avoids writes that would be denied + an orphaned Storage blob). The Sales Dashboard (`AdminStats`) **excludes unclaimed pool leads** from per-rep stats and pipeline totals, surfacing them as a separate **In Prospects** count. **Add a lead** manually via the New Lead form; **bulk CSV upload was removed** in the v1.76.0 overhaul (leads now come from Lead Builder). A pure sales rep (non-admin whose only tool is `sales-crm`) lands on `/sales-crm` directly instead of the dashboard grid (`RootLanding` in `App.tsx`).
- **Lead Builder** (toolId `lead-builder`, admin-only, lives in the **REP** dashboard section) — Builds a qualified, callable C&I sales-lead list for a county. Pipeline: ingest the county tax roll → enrich with Perplexity (operating company/website/description/status) + Apollo (decision-maker + verified email) → review/repair → promote into the `leads` collection **either assigned to a rep or sent to Prospects** (the shared pool — `promoteCompanies(selected, null)` writes `assignedTo: ''`). **Promote UX (v1.82.1):** each Qualified/Needs-review row has a one-click **"→ Prospects"** quick-send (single company, no rep, no scrolling), **clicking a company row opens a read-only Company-detail modal** (v1.82.4 — same look as the Leads-tool LeadDetail: Company info + People info + Enrichment/pipeline sections incl. market value, energy intensity, Perplexity status/confidence, and the status reason; footer has Edit + → Send to prospects; the checkbox still selects), a **Tier filter** (GIANT/BIG/MID/SMALL — v1.82.3) narrows the active tab (select-all / promote / quick-send respect it), and the bulk **promote bar floats at the bottom of the viewport** whenever rows are selected (rep dropdown / "Send to prospects" + Promote), so a long county build no longer means scrolling to the end to act. State + county are dropdowns (`TARGETABLE_REGIONS` in `src/lib/leadPipeline.ts`; gated by having a tax-roll source adapter — **NY only** today, all 57 ORPTS counties). Backend is fully server-side: `ingestCountyTaxRoll` (Firestore trigger on a `lead-pipeline-jobs` doc reaching status `ingesting`; single-flight lease; jobId-scoped deterministic company ids; 0-row → error; **public-power suppression** — parcels in one of NY's 48 municipal-electric systems are dropped at ingest with an `ineligibleReason` and routed to the Dropped tab, since those customers can't choose a supplier and so can't be brokered; matched on `municipality_name` against `MUNI_ELECTRIC` in `ingest.ts`; the 4 rural co-ops + village-in-town precision need a per-address territory resolver, tracked as v2) and the scheduled `processLeadPipeline` (per-job lease against overlapping ticks, parallel chunk enrichment, two admin cost gates, per-stage `counts` tally; a watchdog flips jobs stranded at `ingesting` to `error`). **Drop routing is soft:** only a confidently-closed company is hard-dropped; real-but-unverifiable (e.g. no website) goes to a recoverable `needs_review` bucket. The run page shows a live progress bar while building and a tabbed audit view (Qualified / Needs review / Dropped / Promoted) with per-row reasons, inline repair, promote-to-rep-or-pool, **Re-run** (rebuilds the county in place), **CSV export** (current tab or the whole build, all enrichment fields + reason — `src/utils/exportLeadPipelineCsv.ts`; admin-only tool has no other data egress), and **Retry Apollo** (`retryApolloStage`) — re-runs *only* the Apollo step for rows that errored on it (e.g. an invalid API key), resetting them `dropped_apollo`→`perplexity_done` and re-opening the Apollo cost gate, so Perplexity is never re-charged (vs. Re-run, which wipes + re-enriches everything). **Apollo auth circuit-breaker:** the processor's Apollo stage (`runApolloStage`) halts the build with a clear `error` if a chunk hits ≥3 auth failures (401/403, via `isApolloAuthError`) with zero successes — a bad key would otherwise silently dump a whole county into `dropped_apollo` and report "done". A county has exactly one build. Mobiles are revealed later per-lead in the Leads tool, never in bulk here. Collections `lead-pipeline-jobs` + `lead-pipeline-companies` (admin-only rules). The on-demand reveal callable + webhook live in `functions/src/leadBuilder/phone.ts` (webhook binds the callback to the lead's stored `phoneRequestId` + constant-time token; writes only a mobile-typed number).
- **Sales Dashboard** — Admin-only aggregated view of sales performance. Leaderboard, pipeline breakdown, conversion rates.
- **Bailey Project** and **Construction Projects** — Two instances of the same project-tracker tool, kept separate so the CEO's personal data and the construction team's data never mix. Each instance has overview, team (Owner/GC + subcontractors + supervisors + project managers + labor), tasks, photos, **project folders**, and timeline. Permission levels derived from per-project membership: Admin (global) sees everything; a Supervisor sees and edits projects they're assigned to; Labor sees only assigned projects and can update their own task status + upload photos. Documents live in the scoped **Project folders** `FolderBrowser` (subtree `proj_{jobId}_root`). The legacy category-based **Documents** section (`JobDocumentsSection` + `useJobDocuments` + `constructionDocuments.ts`) was **retired 2026-06-19** — those files are deleted and client access to the old per-job `documents` subcollections is default-denied in `firestore.rules` (rollback window closed 2026-06-13). The `JobDocumentCategory` type is kept only because `DocumentRecord.legacyCategory` still carries it for migrated docs; the old Storage blobs at `construction-documents/{jobId}/…` (and the `-projects-` variant) stay live because migrated folder-system records point at those exact paths, so those storage prefixes remain in `storage.rules`.
...
- **To-Do List** — Collaborative company task list (toolId `todo-list`, route `/todo-list`, available to all authenticated users; **collaborative since v1.61.0**, full-trust model decided 2026-06-12). Every task has a creator (`ownerUid`, **immutable after create** — the update rule pins it so no edit can lock the creator out) and a **single assignee** (`assigneeUid`) — anyone can assign a task to anyone via the user directory (names resolve live through `userLabel`, never cached on the doc). `visibility: 'company' | 'private'`: company-visible tasks are readable **and editable by every authenticated user**; private tasks are creator+assignee only, and **never reach the activity log** (the `onUserTaskWrite` trigger skips them — `/activity` is admin-readable). New tasks default to company except the **Personal category, which defaults private**; legacy docs without the field are treated private until `scripts/migrate-user-tasks.mjs` backfills (it also stamps the queryable `archived` boolean — without it legacy docs are invisible to the bounded listener). Two scope **tabs** (default **Personal**), each with three **view modes** (`tab: 'personal' | 'company'` × `mode: 'list' | 'calendar' | 'board'`; restructured from the old 3-tab My Work / Team / Week — v1.69.0 collapsed it to Personal/Company × List/Week, v1.70.0 renamed Week→Calendar, gave Calendar a Week/Month span toggle, and added the Board). **Personal** = tasks assigned to me; **Company** = company-visible tasks + the viewer's own delegations of any visibility. The three modes:
  - **List** — Personal shows week sections (Overdue/Today/This week/…/No date; done grouped by completion week; Today/Tomorrow relative dates); Company groups per person with initials avatars (person filter + "Assigned by me" delegation filter).
  - **Calendar** — a **Week** or **Month** span, toggled by a `[Week | Month]` button in the nav header (`CalendarNav`, shared by every calendar surface; state `calendarSpan`). **Week span:** Personal = a **days-as-columns** card grid (`PersonalWeekBoard`); Company = the **people × days** meeting board with fullscreen **Present** mode at z-60 (`WeekBoard`, week-only). **Month span:** one shared `MonthCalendar` (Mon-anchored 6×7 grid, adjacent-month days muted, today highlighted, ≤3 chips/day then "+N more") for both tabs — Personal shows my tasks, Company shows company-visible tasks with an **assignee avatar** on each chip. In every calendar surface only **dated tasks** appear (undated work lives in List); chips drag to reschedule (Week-Company also reassigns across people), each chip has a quick done-checkbox (omitted in read-only Present).
  - **Board** — Kanban by status: **To do / In progress / Done** columns (`StatusBoard`, from `ALL_TODO_STATUSES`). Drag a card across columns to set status via `setUserTodoStatus` (stamps/clears `completedAt`); Done cards are draggable here (unlike the calendars). `+ Add` lives in the To do column; Company cards carry an assignee avatar. Columns are built from `tasks.filter(scopeTask)`, so search/category/person filters apply. This is the only first-class home for the **"In progress"** (`doing`) status.
Shared building blocks: `useChipDrag` (generic drop target), `DraggableChip`/`ChipBody` (optional `trailing` avatar; `draggableWhenDone` for the Board), `DroppableCell`, `AddTaskButton`, `CalendarNav`, `makeWeekPlacer`. The filter strip (search/category/person) is shared by List and Board; the to-do/done/archived toggle is List-only. Each task carries a **single `Due` date** (the former "Do on"/`scheduledDate` was merged into `dueDate` 2026-06-15; `scheduledDate` is deprecated, read only via a `dueDate ?? scheduledDate` fallback and folded into `dueDate` on next save). Creation via the **+ New task** window or the Week board's per-cell **+ Add**; clicking a row opens a visual **read view** (category-tinted band, status pill, people row, signal chips), Edit is an explicit step that **writes only changed fields** (no clobbering concurrent teammate edits). Soft archive (`archived` boolean + `archivedAt`; rules `delete: false`); the always-on `or()` subscription (company OR mine OR assigned-to-me, disjuncts mirror the read rule) is narrowed to `archived == false` so it stays bounded — archived tasks load via a separate on-demand subscription. Date math uses calendar-safe `addDays` (never raw 24h multiples — DST). Collection stays `user-tasks` (migration safety); rule in `firestore.rules` + rationale in `docs/firestore-rules.md`. Active tasks sort **overdue → priority (high→normal→low) → soonest date → newest-created**. Lives in the **Company** dashboard section.
...
- **Migration**: One-shot script `scripts/migrate-to-folder-system.mjs` (idempotent, default `--dry-run`, `--confirm` to write) moved every existing CRM doc and construction-job doc/photo into the new schema without touching Storage blobs. The legacy CRM `DocumentsSection` UI was removed 2026-05-27; the construction-side `JobDocumentsSection` UI + hook + `constructionDocuments.ts` lib were retired 2026-06-19. The 30-day rollback window on the legacy `crm-documents` and `construction-jobs/*/documents` collections closed **2026-06-13**, and as of 2026-06-19 client access to the per-job `documents` subcollections is default-denied in `firestore.rules`. The migration was re-run with `--confirm` on 2026-06-19 (and extended to cover `construction-projects-jobs`, which the original 2026-05-14 run never scanned) — it recovered the last documents uploaded through the old UI before retirement; both job collections are now fully migrated (verified 30/30, dry-run reports 0 to create). The two dead activity-log document-write triggers were removed from `functions/src/activity/triggers.ts` on 2026-06-19. The dormant legacy `documents` subcollection docs are now safely purgeable via `scripts/purge-legacy-job-documents.mjs` (dry-run default, `--confirm` to delete) — it deletes a legacy doc ONLY if its migrated `jobDoc_{jobId}_{origId}` copy exists, and never touches Storage blobs (migrated folder-system records still point at the original `construction-documents/{jobId}/…` paths, so those blobs and the `storage.rules` prefix stay).
...
> **Firestore/Storage rules + Cloud Functions deploy hygiene (standing rule, set 2026-06-19):** deploy `firestore.rules`, `storage.rules`, and Cloud Functions **only from `main` after merge** — never `firebase deploy` from a feature-branch working tree. A working-tree deploy on 2026-06-19 put a live `lead-pipeline-companies` admin-write rule into prod that existed in **no** git branch; the next rules deploy from `main` would have silently reverted it. Keep `main` == prod: merge first, then deploy. (App/SPA deploys are automatic via Cloudflare Pages on push to `main`; rules + functions are the manual ones to watch.)
...
src/
  ...
  tools/
    ...
    LeadBuilderIndex.tsx      # Lead Builder index — builds list + new-build form ("/lead-builder", admin-only)
    LeadBuilderRun.tsx        # Lead Builder run page — progress bar + tabbed audit/recovery ("/lead-builder/:jobId")
    ...
    TodoListTool.tsx          # To-Do List ("/todo-list") — collaborative company tasks (assign, delegate, team board)
    ...
    SiteLeadsTool.tsx         # Site Leads ("/site-leads") — inbound landowner submissions review queue; verdict/status filters, promote to Lead
  ...
  hooks/
    ...
    useLeadPipeline.ts        # Lead Builder: jobs list, single job (+companies), job-meta subscriptions
    ...
    useUserTasks.ts           # To-Do List: live tasks subscription + create/assign/archive mutations (user-tasks)
    useMarketFeed.ts          # Market Intelligence: market-intel-feed subscription + setItemStatus
    useSiteLeads.ts           # Site Leads: site-leads subscription + setSiteLeadStatus + promoteSiteLeadToLead
  lib/
    ...
    leadPipeline.ts           # Lead Builder: pipeline CRUD/subscriptions, promote, reasons, TARGETABLE_REGIONS, re-run
    leadPhone.ts              # Lead Builder: revealLeadPhone callable client wrapper ("Grab number")
    ...
    userTasks.ts              # To-Do List Firestore CRUD + or() subscription (user-tasks: company-visible OR owner OR assignee)
    marketIntel.ts            # Market Intelligence: market-intel-feed subscription + read/archive status writes
    siteLeads.ts              # Site Leads: site-leads subscription + status writes + promote-to-lead mapping
...
| `/lead-builder`                   | `LeadBuilderIndex`          | toolId: `lead-builder`         | Lead Builder index — start a county build, list builds (admin-only)                                                   |
| `/lead-builder/:jobId`            | `LeadBuilderRun`            | toolId: `lead-builder`         | Lead Builder run page — progress + tabbed audit/recovery + promote + re-run                                           |
...
| `/todo-list`                      | `TodoListTool`              | all                            | Collaborative to-do list — Personal / Company tabs, each with List / Calendar (Week+Month) / Board views + Present mode (collection `user-tasks`) |
...
| `/site-leads`                     | `SiteLeadsTool`             | toolId: `site-leads`           | Review inbound landowner submissions (collection `site-leads`, written by `/api/public/site-score`); promote to `leads` |
...
2. **Pre-Construction** — Pre-Construction, Site Analyzer, Grid Power Analyzer, Market Intelligence, Site Leads, One-Line Generator
3. **Construction** — Construction Projects
4. **REP** — Leads, Lead Builder _(admin-only)_, Sales Dashboard _(admin-only)_
