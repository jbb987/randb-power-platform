...
- **Migration**: One-shot script `scripts/migrate-to-folder-system.mjs` (idempotent, default `--dry-run`, `--confirm` to write) moved every existing CRM doc and construction-job doc/photo into the new schema without touching Storage blobs. The legacy CRM `DocumentsSection` UI was removed 2026-05-27; the construction-side `JobDocumentsSection` UI + hook + `constructionDocuments.ts` lib were retired 2026-06-19. The 30-day rollback window on the legacy `crm-documents` and `construction-jobs/*/documents` collections closed **2026-06-13**, and as of 2026-06-19 client access to the per-job `documents` subcollections is default-denied in `firestore.rules`. The dormant docs remain in Firestore (and their Storage blobs stay live — migrated folder-system records still point at the original `construction-documents/{jobId}/…` paths) pending an Admin-SDK purge. ⚠️ **Before that purge, re-run the migration with `--confirm`**: the old `JobDocumentsSection` upload path stayed live from the 2026-05-14 migration until its 2026-06-19 retirement, so any job document uploaded in that window exists only in the legacy subcollection and must be migrated forward (the script is idempotent and picks them up by `jobDoc_{jobId}_{origId}` id). The two dead activity-log triggers (`onJobDocumentWrite`, `onConstructionProjectsDocumentWrite` in `functions/src/activity/triggers.ts`) can be dropped along with the purge.
