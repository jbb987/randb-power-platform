...
src/
  ...
  components/
    ...
    crm/                      # Sales CRM (Leads) components
      CrmSidebar.tsx          # Left nav panel (Pipeline + Stats)
      LeadTable.tsx           # Leads table with status-filter chips + search + Location column
      LeadDetail.tsx          # Lead profile modal: location/Maps, notes, status progression, additive contacts/phones, document slots
      LeadForm.tsx            # Create new lead form
      CrmStats.tsx            # Stats dashboard (pipeline, conversion, weekly; per-rep when personal)
  ...
  lib/
    ...
    leads.ts                  # Lead Firestore operations (incl. atomic addLeadArrayItem/removeLeadArrayItem for documents/contacts/phones)
    leadDocuments.ts          # Sales CRM: upload/remove/download lead documents (Storage leads/{leadId}/…, inline documents[] on the lead)
    leadPipeline.ts           # Lead Builder: pipeline CRUD/subscriptions, promote (carries location onto the lead), reasons, TARGETABLE_REGIONS, re-run
