...
- **Leads (Sales CRM)** — Lead management for the sales team. Tracks leads through call/email outreach sequence (New → Call 1 → Email → Call 2 → Final Call → Won/Lost). Leads carry an optional `mobilePhone` revealed on-demand via the **"Grab number"** button (`revealLeadPhone` callable → Apollo async webhook `apolloPhoneWebhook` → writes the decision-maker's direct mobile back). Lead-Builder-sourced leads also carry `tier` / `energyIntensity` / `source: 'lead-builder'` badges.
