...
- **To-Do List** — Collaborative company task list (toolId `todo-list`, route `/todo-list`, available to all authenticated users; **collaborative since v1.61.0**, full-trust model decided 2026-06-12). Every task has a creator (`ownerUid`) and a **single assignee** (`assigneeUid`) — anyone can assign a task to anyone via the user directory (names resolve live through `userLabel`, never cached on the doc). `visibility: 'company' | 'private'`: company-visible tasks are readable **and editable by every authenticated user**; private tasks are creator+assignee only. New tasks default to company except the **Personal category, which defaults private**; legacy docs without the field are treated private (safe migration default). Three person-scoped views — **My tasks** (assigned to me) / **Delegated** (created by me for others) / **Team** (all company-visible, person + category filters) — plus the active/done toggle. Hard delete replaced by **soft archive** (`archivedAt` + Archived view with Restore); the Firestore rule sets `delete: false`. Client subscribes with an `or()` query (company-visible OR mine OR assigned-to-me) whose disjuncts mirror the read rule exactly (no composite index needed). Collection stays `user-tasks` (migration safety); rule documented in `docs/firestore-rules.md`; `onUserTaskWrite` activity trigger audits every create/edit/reassign/completion. Active tasks sort **overdue → priority (high→normal→low) → soonest date → newest-created**; done tasks show most-recently-completed first. Lives in the **Company** dashboard section.
...
| `/todo-list`                      | `TodoListTool`              | all                            | Collaborative to-do list — My/Delegated/Team views (collection `user-tasks`)                                           |
