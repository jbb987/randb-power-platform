...
- **Grid Power Analyzer** — Interactive MapLibre GL map showing power generators, transmission lines, substations, and available capacity with heat map overlay. Coordinate search with gold diamond pin. The substation popup includes a **Ring bus estimate** card (`RingBusCard` + pure `src/lib/ringBus.ts`): the user counts breakers on the popup's Aerial view links; since a ring bus holds one breaker per circuit element, transformers ≈ breakers − HIFLD line count, sized by class-typical MVA into a screening transformation-capacity range (Bailey field rule, 2026-07-02). Counts persist one-doc-per-substation in the `substation-field-counts` Firestore collection (`src/lib/fieldCounts.ts`, doc id `hifld_{id}` / coord fallback; authed read/write rule in firestore.rules — deploy rules from main after merge): a saved count prefills the card for the whole team with a 'Saved by · date' stamp; the estimate itself is always recomputed from the inputs, never stored. Methodology section 2 documents the rule + limitations.
